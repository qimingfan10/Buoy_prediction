...
class TCNClassifier(nn.Module):
    def __init__(self, input_size=5, num_channels=[64, 128, 256], kernel_size=3, dropout=0.2):
        super().__init__()
        layers = []
    ...
    def forward(self, x, lengths=None):
        ...
        # TCN特征提取
        x = self.tcn(x)  # [batch_size, channels, seq_len]
        
        if lengths is not None:
            # 使用真实长度进行掩码
            mask = torch.arange(x.size(2), device=x.device)[None, :] < lengths[:, None]
            mask = mask.float().unsqueeze(1)  # [batch_size, 1, seq_len]
            x = (x * mask).sum(dim=2) / lengths.float().unsqueeze(1)
        else:
